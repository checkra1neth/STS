...
Скрипт автоматически скачает и закеширует модель `distil-small.en`, которая потребляет значительно меньше ресурсов, но при этом обеспечивает высокое качество для английской и частично многоязычной речи. При необходимости можно выбрать другую модель, например `small` или `medium`, указав флаг `--model`.
...
При необходимости можно настроить количество потоков CPU (`--cpu-threads` ограничивает количество внутренних потоков faster-whisper), размер `beam search`, температуру, язык распознавания и отключить VAD-фильтр.
...
При первом запуске выбранная модель Whisper будет скачана и сохранена в кэш директории `~/.cache/faster-whisper`. Убедитесь, что у вас достаточно свободного места (для `distil-small.en` требуется около 1 ГБ).
