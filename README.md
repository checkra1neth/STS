...
### Потоковая транскрибация и стратегии буферизации

Режим `--stream` транскрибирует звук в реальном времени и использует *буферный менеджер* для повышения устойчивости текста.

- **Стратегия предложений (`sentence`)** — значение по умолчанию. Менеджер ждёт завершения предложения (пунктуация `.?!…`) или длинной паузы между сегментами, прежде чем зафиксировать текст. Это чуть увеличивает задержку, но заметно снижает рывки и самоповторы.
- **Стратегия сегментов (`segment`)** — выдаёт каждую реплику модели сразу. Задержка минимальна, однако результирующий текст чаще «прыгает», особенно при перекрытии чанков.

Чтобы избежать зависания незавершённых предложений, по умолчанию включена автоматическая очистка буфера: если пауза длится дольше заданного окна, текущий текст принудительно выводится. Поведение можно настроить:

```bash
python -m sts.cli --stream --buffer-trimming --buffer-trimming-sec 4
```

- `--buffer-trimming/--no-buffer-trimming` — включить или отключить принудительный сброс буфера.
- `--buffer-trimming-sec` — сколько секунд ждать перед выводом неполного текста. Меньшие значения уменьшают задержку, большие делают вывод стабильнее.
